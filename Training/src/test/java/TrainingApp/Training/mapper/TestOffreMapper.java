...
public class TestOffreMapper extends AbstractTrainingTest {
	...
	public void testGetOffreByIdSuccess() throws Exception {
		...
		Assertions.assertThat(offre.getProduit().getId()).isNotNull().isPositive()
				.isEqualTo(offreRef.getProduit().getId());
		...
		Assertions.assertThat(offre.getVendeur().getId()).isNotNull().isPositive()
				.isEqualTo(offreRef.getVendeur().getId());
		Assertions.assertThat(offre.getVendeur().getNomEntreprise())
				.isEqualTo(offreRef.getVendeur().getNomEntreprise());
		Assertions.assertThat(offre.getVendeur().getUsrId()).isNotNull().isPositive()
				.isEqualTo(offreRef.getVendeur().getUsrId());
	}
